		...
		Context(GPUContext), Device(dev), CommandQueue(CommandQue)
{
	/*** get input data ***/
#ifdef _MOBILE
	char input_train_file_name[1024] = "/storage/emulated/0/gksvm/b9f3t.scale";
	char input_test_file_name[1024] = "/storage/emulated/0/gksvm/b9f3p.scale";
#else
	char input_train_file_name[1024] = "../0.data/data/b9f3t.s";
	char input_test_file_name[1024] = "../0.data/data/b9f3p.s";
#endif
	//in order to init h_train, assign value to nFeats,h_train and nTests
	init_variable_from_prob(input_train_file_name, &nTrains, 0); //0:lmt
	init_variable_from_prob(input_test_file_name, &nTests, 0);

	cl_int err;
	size_t size_trains = sizeof(float)*nTrains*nFeats;
	size_t size_tests = sizeof(float)*nTests*nFeats;

	d_train	= clCreateBuffer(Context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, size_trains, NULL, &err);
	if(err != CL_SUCCESS) debug("d_train failed to create!\n");
	d_test	= clCreateBuffer(Context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, size_tests, NULL, &err);
	if(err != CL_SUCCESS) debug("d_test failed to create!\n");

	h_train = (float*)clEnqueueMapBuffer(CommandQueue, d_train, CL_TRUE, CL_MAP_WRITE, 0, size_trains, 0, NULL, NULL, &err);
	if(err != CL_SUCCESS) debug("Cannot map from d_train to h_train");
	h_test = (float*)clEnqueueMapBuffer(CommandQueue, d_test, CL_TRUE, CL_MAP_WRITE, 0, size_tests, 0, NULL, NULL, &err);
	if(err != CL_SUCCESS) debug("Cannot map from d_test to h_test");
	
	read_problem(input_train_file_name, h_train, nTrains, 1); //column major matrix
	...
	for(uint i=0; i<nTests/nTests_inWork; i++)
	{
		...
		h_test = (float*)clEnqueueMapBuffer(CommandQueue, d_test, CL_TRUE, CL_MAP_READ, 0, size_tests, 0, NULL, NULL, &err);
		if(err != CL_SUCCESS) debug("Cannot map from d_test to h_test");
		h_train = (float*)clEnqueueMapBuffer(CommandQueue, d_train, CL_TRUE, CL_MAP_READ, 0, size_trains, 0, NULL, NULL, &err);
		if(err != CL_SUCCESS) debug("Cannot map from d_train to h_train");
...
void CLKnn::PreProcess(void)
{
	...
	string fileDir;
	fileDir.append("/storage/emulated/0/gksvm/kernel_psvmknn.cl");
	...
	ckRadixsort 		= clCreateKernel(Program, "radixsort", &err);
	...
	debug("create buffer");
	size_t num_distances = nTests_inWork*nTrains_rounded;
	d_inIdx		= clCreateBuffer(Context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, sizeof(htype)*num_distances, NULL, &err);
	d_outIdx	= clCreateBuffer(Context, CL_MEM_READ_WRITE, sizeof(htype)*num_distances, NULL, &err);
	d_inDist 	= clCreateBuffer(Context, CL_MEM_READ_WRITE, sizeof(float)*num_distances, NULL, &err);
	d_inLabel 	= clCreateBuffer(Context, CL_MEM_READ_WRITE, sizeof(uchar)*num_distances, NULL, &err);
	d_outDist 	= clCreateBuffer(Context, CL_MEM_READ_WRITE, sizeof(float)*num_distances, NULL, &err);
	d_outLabel 	= clCreateBuffer(Context, CL_MEM_READ_WRITE, sizeof(uchar)*num_distances, NULL, &err);
...
void CLKnn::CLDistance(uint idx)
{
	...
	err = clSetKernelArg(ckDist, 4, sizeof(uint), &nTests_inWork);
	...
	if(err != CL_SUCCESS) debug("Cannot set kernel argument of CLDistance");
...
void CLKnn::CLRadixSort(uint pass)
{
	cl_int err;
	cl_event eve;
	size_t globalSize = nTests_inWork*split;//m*64
	size_t localSize = WRAP;//64

	err = clSetKernelArg(ckRadixsort, 0, sizeof(cl_mem), &d_inDist);
	err = clSetKernelArg(ckRadixsort, 1, sizeof(cl_mem), &d_inLabel);
	err = clSetKernelArg(ckRadixsort, 2, sizeof(cl_mem), &d_outDist);
	err = clSetKernelArg(ckRadixsort, 3, sizeof(cl_mem), &d_outLabel);
	err = clSetKernelArg(ckRadixsort, 4, sizeof(cl_mem), &d_inIdx);
	err = clSetKernelArg(ckRadixsort, 5, sizeof(cl_mem), &d_outIdx);
	err = clSetKernelArg(ckRadixsort, 6, sizeof(htype)*localSize*radix, NULL); //4096byte
	err = clSetKernelArg(ckRadixsort, 7, sizeof(htype)*radix, NULL); //32*2byte
	err = clSetKernelArg(ckRadixsort, 8, sizeof(uint), &pass);
	err = clSetKernelArg(ckRadixsort, 9, sizeof(uint), &radixbits);//5
	err = clSetKernelArg(ckRadixsort, 10, sizeof(uint), &radix);
	err = clSetKernelArg(ckRadixsort, 11, sizeof(uint), &nTrains_rounded);

	if(err != CL_SUCCESS) debug("cannot set kernel argument of Histogram");
	if(sizeof(htype)*(localSize*radix + radix) >= localMem)
			debug("local memory is not enough for Histogram");

	err = clEnqueueNDRangeKernel(CommandQueue, ckRadixsort, 1, NULL, &globalSize, &localSize, 0, NULL, &eve);
...
void CLKnn::Predict(uint idx)
{
	...
	if(err != CL_SUCCESS) debug("Cannot set kernel argument of ckClassifyHist");
